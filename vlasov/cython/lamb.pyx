...
def derivative_lambda_tensor(complex_t omega, real_t kpar, real_t kperp,
                             species_t s):
    ...
    cdef complex_t dT[9]
    cdef complex_t a[9]
    cdef complex_t zeta_0, zeta_n, square, d_square, dW, Wn, temp, d_zeta_n
    cdef real_t Gam, GamP, lamb, lamb_sqrt
    ...
    for j in range(1, 2*(N+1)):
        ...
        Gam = ive(n, lamb)
        GamP = 0.5*(ive(n-1, lamb) + ive(n+1, lamb)) - ive(n, lamb)
        dW = -(zeta_n*Wn + (Wn + 1.0)/zeta_n)

        square = omega/(omega-n*s.oc)*(1.0-Wn) + s.Delta*Wn
        d_square = -(1.0 - Wn)*n*s.oc/(omega - n*s.oc)**2.0
        d_square += (s.Delta - omega/(omega - n*s.oc))*dW/(kpar*s.vt)
        T[0] = n**2*Gam/lamb
        T[1] = 1j*n*GamP
        T[2] = n*Gam/sqrt(lamb)*zeta_n/sqrt(1-s.Delta)
        T[3] = T[1].conjugate()
        ...
        dT[0] = 0.0
        dT[1] = 0.0
        dT[2] = n*Gam/sqrt(lamb)*d_zeta_n/sqrt(1-s.Delta)
        dT[3] = 0.0
        dT[4] = 0.0
        dT[5] = -1j*sqrt(lamb)*GamP*d_zeta_n/sqrt(1-s.Delta)
        dT[6] = T[2].conjugate()
        dT[7] = T[5].conjugate()
        dT[8] = Gam*2.0*d_zeta_n*zeta_n/(1-s.Delta)

        count = 0
        for i in range(9):
            temp = d_square*T[i] + square*dT[i]
            if j > 1:
                if abs(temp/a[i]) > tol:
                    a[i] += temp
                    count += 1
    ...
    a[8] -= 1/(1-s.Delta)*2.0*d_zeta_n*zeta_0
